...
Todo:
    ...
    * http://www.cgdzg.com/maya-api-custom-locator-node/
    * https://github.com/alicevision/mayaAPI/blob/master/2016.sp1/linux/devkit/plug-ins/scripted/pyFootPrintNode.py
    * https://forums.cgsociety.org/t/world-matrix-in-compute-function-of-mpxlocatornode/1608427
...
class DebugPoleVector(omui2.MPxLocatorNode):
    ...
    def postConstructor(self):
        ...
        om2.MFnDependencyNode(thisMob).setName("gfDebugPv_PShape#")

    @staticmethod
    def creator():
        """ Maya creator function. """
        return DebugPoleVector()

    @staticmethod
    def initialize():
        """
        Defines the set of attributes for this node. The attributes declared in this function are assigned
        as static members to DebugPoleVector class. Instances of DebugPoleVector will use these attributes to create plugs
        for use in the compute() method.
        """
        uAttr = om2.MFnUnitAttribute()

        DebugPoleVector.inSize = uAttr.create("size", "sz", om2.MFnUnitAttribute.kDistance)
        uAttr.default = om2.MDistance(1.0)
        # INPUT_ATTR(uAttr)

        DebugPoleVector.addAttribute(DebugPoleVector.inSize)
    ...
    def draw(self, view, path, style, status):
        ...
        # Get the size
        thisMob = self.thisMObject()
        plug = om2.MPlug(thisMob, DebugPoleVector.inSize)
...
class DebugPoleVectorDrawOverride(omr2.MPxDrawOverride):
    ...
    def getMultiplier(self, objPath):
        ...
        node = objPath.node()
        plug = om2.MPlug(node, DebugPoleVector.inSize)
        if not plug.isNull:
            sizeVal = plug.asMDistance()
